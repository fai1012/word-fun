...
import { User as UserIcon, Plus, ChevronRight, Pencil, Trash2, AlertTriangle, X, Check } from 'lucide-react';
                ...
                <div className="grid grid-cols-1 gap-4 mb-8">
                    {profiles.filter(p => p && p.id).map(profile => (
                        <div key={profile.id} className="group relative">
                            <div
                                onClick={() => !isEditMode && onProfileSelect(profile)}
                                className={`w-full bg-white p-4 rounded-3xl shadow-[4px_4px_0px_0px_rgba(93,64,55,0.2)] border-2 border-coffee flex items-center gap-4 transition-all text-left ${isEditMode ? 'pr-24 cursor-default' : 'hover:shadow-[4px_4px_0px_0px_rgba(93,64,55,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] cursor-pointer active:translate-x-[2px] active:translate-y-[2px] active:shadow-none'}`}
                            >
                                {/* Avatar Circle */}
                                <div className="relative group/avatar">
                                    ...
                                    {isEditMode && (
                                        <button
                                            ...
                                            className="absolute -bottom-1 -right-1 bg-white rounded-full p-1.5 shadow-md border-2 border-coffee hover:bg-salmon hover:text-white hover:border-salmon transition-colors"
                                        >
                                ...
                                {!isEditMode && <ChevronRight className="w-6 h-6 text-coffee/20 group-hover:text-salmon shrink-0 stroke-[3]" />}
                            </div>

                            <div className={`absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2 transition-opacity ${isEditMode ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                    ...
                    <button
                        ...
                        className={`p-4 rounded-3xl border-2 border-dashed flex items-center gap-4 transition-all text-left group ${isEditMode ? 'bg-indigo-50 border-indigo-300 text-indigo-600' : 'bg-coffee/5 border-coffee/30 text-coffee/60 hover:bg-white hover:border-indigo-500 hover:text-indigo-500'}`}
                    >
                        <div className={`w-14 h-14 rounded-full flex items-center justify-center border-2 shrink-0 transition-colors ${isEditMode ? 'bg-indigo-500 text-white border-indigo-600' : 'bg-white text-coffee/40 border-coffee/20 group-hover:border-indigo-500 group-hover:bg-indigo-500 group-hover:text-white'}`}>
